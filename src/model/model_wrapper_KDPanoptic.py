# This is the file that implement the kd for the panoptic dataset
...
class ModelWrapper_KDPanoptic(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 2. Randomly sample `gaussians_count` Gaussians
...
def batched_greedy_match(sampled, refined):
    """
    sampled: (bs, N, 3)
    refined: (bs, N, 3)
    Returns:
        indices: (bs, N) - for each refined, the closest sampled
    """
    # Compute squared distances in a memory-efficient way
    # dist[i,j,k] = ||refined[i,j] - sampled[i,k]||^2

    bs, N, _ = sampled.shape

    # ||a - b||^2 = ||a||^2 + ||b||^2 - 2*a.b
    refined_sq = (refined ** 2).sum(dim=2, keepdim=True)         # (bs, N, 1)
    sampled_sq = (sampled ** 2).sum(dim=2).unsqueeze(1)          # (bs, 1, N)
    cross = torch.bmm(refined, sampled.transpose(1, 2))          # (bs, N, N)

    dists = refined_sq + sampled_sq - 2 * cross                  # (bs, N, N)

    indices = torch.argmin(dists, dim=2)  # (bs, N)
    return indices

        
def greedy_match(sampled, refined):
    bs, N, D = sampled.shape
    matched_indices = []
    for b in range(bs):
        dist = torch.cdist(refined[b], sampled[b])  # (N, N)
